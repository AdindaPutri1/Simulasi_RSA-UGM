...
pygame.display.set_caption("Simulasi Antrian Pasien dengan Satu Loket")
...
def main():
    ...
    while running:
        ...
        # Update status loket
        if current_patient_at_loket is None and waiting_queue:
            # Check waiting time
            if pygame.time.get_ticks() >= waiting_times[id(waiting_queue[0])]:
                current_patient_at_loket = waiting_queue.pop(0)
                current_patient_at_loket['target'] = POS_LOKET_PENDAFTARAN
                time_at_loket_end = pygame.time.get_ticks() + (random.randint(2000, 5000) // SPEED_FACTOR)  # Waktu random di loket pendaftaran
                current_patient_at_loket['wait_start'] = pygame.time.get_ticks()
                del waiting_times[id(current_patient_at_loket)]

        if current_patient_at_loket:
            move_patient(current_patient_at_loket, POS_LOKET_PENDAFTARAN, SPEED * SPEED_FACTOR)

            if (current_patient_at_loket['x'] == POS_LOKET_PENDAFTARAN[0] and
                current_patient_at_loket['y'] == POS_LOKET_PENDAFTARAN[1]):
                if current_patient_at_loket['waiting'] == False:
                    current_patient_at_loket['waiting'] = True
                    current_patient_at_loket['wait_start'] = pygame.time.get_ticks()
                    current_patient_at_loket['wait_duration'] = (random.randint(2000, 5000) // SPEED_FACTOR)  # Waktu random menunggu di loket
                elif pygame.time.get_ticks() - current_patient_at_loket['wait_start'] >= current_patient_at_loket['wait_duration']:
                    if current_patient_at_loket['color'] == GREEN:
                        current_patient_at_loket['target'] = POS_RUANG_PEMERIKSAAN
                    else:
                        current_patient_at_loket['target'] = (-50, current_patient_at_loket['y'])  # Keluar layar ke kiri
                    current_patient_at_loket['waiting'] = False
                    current_patient_at_loket = None
        ...
        for idx, patient in enumerate(waiting_queue):
            ...
            if patient['target'] != POS_LOKET_PENDAFTARAN:
        ...
        pygame.draw.rect(screen, (200, 200, 200), (*POS_LOKET_PENDAFTARAN, 125, 50))
        ...
        # Gambar teks ruangan
        draw_text("Loket 1", (POS_LOKET_PENDAFTARAN[0], POS_LOKET_PENDAFTARAN[1] - 40))
