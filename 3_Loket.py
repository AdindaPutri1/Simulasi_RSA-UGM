...
POS_LOKET_PENDAFTARAN_3 = (350, 70)
...
pygame.display.set_caption("Simulasi Antrian Pasien dengan Tiga Loket")
...
def main():
    ...
    # Status antrian
    loket_empty = [True, True, True]
    waiting_queue = []
    waiting_times = {}
    current_patient_at_loket = [None, None, None]
    time_at_loket_end = [0, 0, 0]
    ...
    while running:
        ...
        # Update status loket
        for i in range(3):
            if current_patient_at_loket[i] is None and waiting_queue:
                if pygame.time.get_ticks() >= waiting_times[id(waiting_queue[0])]:
                    current_patient_at_loket[i] = waiting_queue.pop(0)
                    if i == 0:
                        current_patient_at_loket[i]['target'] = POS_LOKET_PENDAFTARAN_1
                    elif i == 1:
                        current_patient_at_loket[i]['target'] = POS_LOKET_PENDAFTARAN_2
                    else:
                        current_patient_at_loket[i]['target'] = POS_LOKET_PENDAFTARAN_3
                    time_at_loket_end[i] = pygame.time.get_ticks() + (random.randint(2000, 5000) // SPEED_FACTOR)
                    current_patient_at_loket[i]['wait_start'] = pygame.time.get_ticks()
                    del waiting_times[id(current_patient_at_loket[i])]

            if current_patient_at_loket[i]:
                if i == 0:
                    move_patient(current_patient_at_loket[i], POS_LOKET_PENDAFTARAN_1, SPEED * SPEED_FACTOR)
                elif i == 1:
                    move_patient(current_patient_at_loket[i], POS_LOKET_PENDAFTARAN_2, SPEED * SPEED_FACTOR)
                else:
                    move_patient(current_patient_at_loket[i], POS_LOKET_PENDAFTARAN_3, SPEED * SPEED_FACTOR)

                if ((i == 0 and current_patient_at_loket[i]['x'] == POS_LOKET_PENDAFTARAN_1[0] and current_patient_at_loket[i]['y'] == POS_LOKET_PENDAFTARAN_1[1]) or
                    (i == 1 and current_patient_at_loket[i]['x'] == POS_LOKET_PENDAFTARAN_2[0] and current_patient_at_loket[i]['y'] == POS_LOKET_PENDAFTARAN_2[1]) or
                    (i == 2 and current_patient_at_loket[i]['x'] == POS_LOKET_PENDAFTARAN_3[0] and current_patient_at_loket[i]['y'] == POS_LOKET_PENDAFTARAN_3[1])):
                    if not current_patient_at_loket[i]['waiting']:
                        current_patient_at_loket[i]['waiting'] = True
                        current_patient_at_loket[i]['wait_start'] = pygame.time.get_ticks()
                        current_patient_at_loket[i]['wait_duration'] = (random.randint(2000, 5000) // SPEED_FACTOR)
                    elif pygame.time.get_ticks() - current_patient_at_loket[i]['wait_start'] >= current_patient_at_loket[i]['wait_duration']:
                        if current_patient_at_loket[i]['color'] == GREEN:
                            current_patient_at_loket[i]['target'] = POS_RUANG_PEMERIKSAAN
                        else:
                            current_patient_at_loket[i]['target'] = (-50, current_patient_at_loket[i]['y'])  # Keluar layar ke kiri
                        current_patient_at_loket[i]['waiting'] = False
                        current_patient_at_loket[i] = None
        ...
        for idx, patient in enumerate(waiting_queue):
            ...
            if patient['target'] not in [POS_LOKET_PENDAFTARAN_1, POS_LOKET_PENDAFTARAN_2, POS_LOKET_PENDAFTARAN_3]:
        ...
        pygame.draw.rect(screen, (200, 200, 200), (*POS_LOKET_PENDAFTARAN_3, 125, 50))
        ...
        draw_text("Loket 3", (POS_LOKET_PENDAFTARAN_3[0], POS_LOKET_PENDAFTARAN_3[1] - 40))
